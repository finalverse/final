...
class _OgreWorldClassExport World_TerrainTessellation : public SdkWorld
{
public:

    World_TerrainTessellation()
        : mTerrainGroup(0)
        , mTerrainPaging(0)
        , mPageManager(0)
        //, mFallVelocity(0)
        ...
        , mTerrainPos(1,1,1)
    {
        ...
        mInfo["Description"] = "World for terrain tessellation and the use of displacement mapping";
    ...
    void testCapabilities(const RenderSystemCapabilities* caps)
    {
        OGRE_EXCEPT(Exception::ERR_NOT_IMPLEMENTED, "This sample is not yet finished."
                " Sorry!", "World_TerrainTessellation::testCapabilities");
        if (!caps->hasCapability(RSC_TESSELLATION_PROGRAM))
        {
            OGRE_EXCEPT(Exception::ERR_INVALID_STATE, "Your graphics card does not support tesselation shaders. Sorry!",
                "World_TerrainTessellation:testCapabilities");
        }
            ...
            !GpuProgramManager::getSingleton().isSyntaxSupported("hlsl"))
        {
            OGRE_EXCEPT(Exception::ERR_NOT_IMPLEMENTED, "Your card does not support the shader model 5.0 needed for this sample, "
                "so you cannot run this sample. Sorry!", "World_TerrainTessellation::testCapabilities");
        }
    }
