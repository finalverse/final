...
void World_MeshLod::setupContent()
{
...
void World_MeshLod::cleanupContent()
{
...
void World_MeshLod::setupControls( int uimode /*= 0*/ )
{
    cleanupControls();
...
void World_MeshLod::changeSelectedMesh( const String& name )
{
    if(mMeshEntity){
        mSceneMgr->destroyEntity(mMeshEntity);
        mMeshEntity = 0;
...
void World_MeshLod::loadAutomaticLod()
{
...
void World_MeshLod::loadUserLod( bool useWorkLod )
{
...
void World_MeshLod::forceLodLevel(int lodLevelID, bool forceDelayed)
{
...
size_t World_MeshLod::getUniqueVertexCount( MeshPtr mesh )
{
...
void World_MeshLod::addLodLevel()
{
...
void World_MeshLod::loadLodLevel( int id )
{
...
void World_MeshLod::removeLodLevel()
{
    if(mLodConfig.levels.empty()){
        return;
    }
...
void World_MeshLod::removeInitialLodLevel()
{
...
Real World_MeshLod::getCameraLODValue()
{
...
void World_MeshLod::moveCameraToPixelDistance( Real pixels )
{
...
bool World_MeshLod::getResourceFullPath(MeshPtr& mesh, String& outPath)
{
...
void World_MeshLod::addToProfile( Real cost )
{
    LodConfig config(mLodConfig);
    config.levels.clear();
    config.levels.push_back(mWorkLevel);
...
bool World_MeshLod::frameStarted( const FrameEvent& evt )
{
    mDistanceLabel->setCaption(StringUtil::format("Pixel Count: %d px", int(getCameraLODValue())));
    return true;
}

void World_MeshLod::checkBoxToggled( CheckBox * box )
{
...
void World_MeshLod::itemSelected( SelectMenu* menu )
{
...
void World_MeshLod::sliderMoved(Slider* slider)
{
...
void World_MeshLod::buttonHit( OgreBites::Button* button )
{
...
bool World_MeshLod::shouldInject( LodWorkQueueRequest* request )
{
    return true;
}

void World_MeshLod::injectionCompleted( LodWorkQueueRequest* request )
{
    recreateEntity(); // Needed for manual lod levels.
