 ...
 */
class _OgreWorldClassExport SSAOGBufferSchemeHandler : public Ogre::MaterialManager::Listener
{
public:
...
class _OgreWorldClassExport World_SSAO : public SdkWorld
{
private:
...
protected:
    ...
    void setupCompositors()
    {
        ...
        if (CompositorManager::getSingleton().addCompositor(mViewport, "SSAO/GBuffer"))
            ...
            LogManager::getSingleton().logMessage("World_SSAO: Failed to add GBuffer compositor\n");
        ...
        for (unsigned int i = 0; i < mCompositorNames.size(); i++)
        {
            if (CompositorManager::getSingleton().addCompositor(mViewport, mCompositorNames[i]))
                ...
                LogManager::getSingleton().logMessage("World_SSAO: Failed to add compositor: " + mCompositorNames[i] + "\n");
        }
        ...
        for (unsigned int i = 0; i < mPostNames.size(); i++)
        {
            ...
            if (CompositorManager::getSingleton().addCompositor(mViewport, mPostNames[i]))
                ...
                LogManager::getSingleton().logMessage("World_SSAO: Failed to add " + mPostNames[i] + " compositor\n");
        }
    ...
    void setupControls(void)
    {
        ...
        mTrayMgr->createCheckBox(TL_TOPLEFT, SSAO_SAMPLE_SPACE_NAME, "World in Screen Space", SSAO_GUI_WIDTH);
        mTrayMgr->createThickSlider(TL_TOPLEFT,
    ...
    void sliderMoved(Slider* slider) override
    {
        ...
        {
            setUniform("SSAO/Crytek", "cWorldLengthScreenSpace", slider->getValue()/100.0f);
            setUniform("SSAO/HorizonBased", "cWorldLengthScreenSpace", slider->getValue()/100.0f);
            setUniform("SSAO/HemisphereMC", "cWorldLengthScreenSpace", slider->getValue()/100.0f);
            setUniform("SSAO/Volumetric", "cWorldLengthScreenSpace", slider->getValue()/100.0f);
        }
        else if (slider->getName() == SSAO_SAMPLE_LENGTH_WORLDSPACE)
        {
            setUniform("SSAO/Crytek", "cWorldLengthWorldSpace", slider->getValue());
            setUniform("SSAO/HorizonBased", "cWorldLengthWorldSpace", slider->getValue());
            setUniform("SSAO/HemisphereMC", "cWorldLengthWorldSpace", slider->getValue());
            setUniform("SSAO/Volumetric", "cWorldLengthWorldSpace", slider->getValue());
        }
        ...
        else if (slider->getName() == SSAO_BILATERAL_PHOTOMETRIC_EXPONENT)
        {
            ...
            setUniform("SSAO/HemisphereMC", "cWorldLengthExponent", slider->getValue());
    }
    ...
    void checkBoxToggled(OgreBites::CheckBox *box) override
    {
        ...
        {
            setUniform("SSAO/Crytek", "cWorldInScreenspace", box->isChecked());
            setUniform("SSAO/HorizonBased", "cWorldInScreenspace", box->isChecked());
            setUniform("SSAO/HemisphereMC", "cWorldInScreenspace", box->isChecked());
            setUniform("SSAO/Volumetric", "cWorldInScreenspace", box->isChecked());
